...
const navItems = [
  ...
  { label: 'Borrow Requests', icon: 'pi pi-clock', path: '/handler/borrow-requests' },
  { label: 'Reports', icon: 'pi pi-chart-bar', path: '/handler/reports' },
];

export function HandlerBorrowRequests() {
          ...
          <nav className="nav-list" aria-label="Admin navigation">
            {navItems.map((item) => (
              <button
                key={item.path}
                type="button"
                className={`nav-btn ${item.path === '/handler/borrow-requests' ? 'is-active' : ''}`}
